...
# docker volume create ipfsdata-$BCM_BITCOIN_CHAIN

# docker run -d --name bootstrapper -v bitcoind-$BCM_BITCOIN_CHAIN-data:/bitcoindata -v ipfsdata-$BCM_BITCOIN_CHAIN:/data/ipfs ipfs/go-ipfs:latest daemon
...
# # we'll run the ipfs listenner inside the bitcoin app. It runs whether user wants it or not.
# #lxc exec bitcoin -- docker run -d --name bootstrapper -v bitcoind-$BITCOIN_CHAIN-data:/bitcoindata -v ipfsdata-$BITCOIN_CHAIN:/ipfsdata -e IPFS_PATH=/ipfsdata ipfs/go-ipfs daemon
